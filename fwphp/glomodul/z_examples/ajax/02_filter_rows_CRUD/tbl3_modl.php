...
//see <tbody id="t blrows"></tbody> and $("#t blrows").html(tbody_ data);
...
if(isset($_POST['name'])) 
{
    ...
    ************************************************************ -->*/
    $name = $_POST['name'];
    $query = "INSERT INTO posts(title) VALUES('$name') ";
...
if(isset($_POST['id'])) 
{
  ...
  if(isset($_POST['changerow'])) 
  {
    ...
    if ($title) $set = "title='".$title."'"; else $set = '';
    if ($user_id) {if ($title) $set = $set.','; $set = $set."user_id='".$user_id."'";}

    $query = "UPDATE posts SET $set WHERE id = $idposted ";
                          //$query = "UPDATE post SET title='$title', user_id='$user_id' WHERE id = $idposted ";
     $result_set = mysqli_query($connection, $query);
     if(!$result_set) {
         die("U P D A T E  F A I L E D" . mysqli_error($connection));
     } 
  } //isset($_POST['c h a n g e r o w']
     //works: $query = "UPDATE post SET title = 'Kalendar original' WHERE id = 26 ";

   /*<!-- ************************************************************
         4. D E L E T E - code behind R O W  d e l  b u t t o n
   ************************************************************ -->*/
  if(isset($_POST['deleterow'])) {
      $query = "DELETE FROM posts WHERE id = $idposted ";
      $result_set = mysqli_query($connection, $query);
      if(!$result_set) {
