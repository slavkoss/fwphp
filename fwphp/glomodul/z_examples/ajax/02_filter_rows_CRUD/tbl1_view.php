...
<div class="container">
    ...
    ***************************************************************** -->
        <div class="col-12">
          <table class="table">
             <thead><tr><th>Id</th><th>Post (event) title</th><th>User id</th></tr></thead>
...
<?php
   ftr_pge('jquery.min.js', ''); //zepto.min.js


   //ob_start();
   //include($path);
   //$t blrows = ob_get_clean(); //echo $t blrows ;
...
?>
    ...
    <script LANGUAGE="JavaScript">
      ...
      {
                      /*if ($('#t blrows').length !== 0) {
                        v_tblr = $('#t blrows');
                        v_tblr
                          //.hide()
                          .html(
                             //'WORKS : aaaaaaaaaaaaaaaaa '
                             "<=substr($t blrows,0,100) //WORKS BUT AFTER 100 IS SOMETHING BAD >"
                           )
            ...
            //function TblDispay(){
                ...
                $.ajax({ // AJAX CHANGES ONLY TBODY PAGE PART !!
                  ...
                  success: function(tbody_data){ 
                    //tbody_ data is from AJAX server script tbl3_modl.php
...
{ ?>
    ...
    <?php if ($jquery_lib) { ?> 
       <script src="/zinc/themes/bootstrap/js/<?=$jquery_lib?>"></script><?php
    } ?>
