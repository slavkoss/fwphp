...
?>
  ...
  <?php 
    $dbFile='blog.db'; // D:\web\blog\protected\data\blog.db
    $dsn = 'sqlite:blog.db'; //Data Source Name ide i sqlite2, ne ide 'sqlite:dbname=blog;host=127.0.0.1'; 
    $usr = ''; $psw = '';
    //@unlink($dsn);
    try { $conn = new PDO($dsn, $usr, $psw);
...
<p>--------------------------------------------------------
  ...
  <?php 
    //try {
    $sqlObj = sqlite_query($conn, $sql);   // sqlite_query() expects parameter 1 to be resource, object given
...
<tr>
<td>
<h2>2. SQLite DBMS DBI ez_sql</h2>
<?php
