<?php
// H:\dev_web\htdocs\t_oci8\ACXE2\db.inc.php
namespace Oracle;
//require('confDb.php');
define('SCHEMA',               'hr');
define('PASSWORD',             'hr');
// DB c o n n  i d
define('DATABASE',             'sspc/XE:pooled');
...
class Db {
    ...
    function __construct($module, $cid) {
        ...
        // Record web user "name", client info and module.
...
// Run a SQL or PL/SQL statement
    public function execute($sql, $action, $bindvars = array()) {
        ...
        //Unlike conn. errors where oci_error() takes no argument, to 
        //check errors from oci_ parse() pass conn. resource to oci_ error():
        if (!$this->stid) {
