...
$langues = array(
	'en' => array(
		...
		'txtNoProjet' => 'No projects yet.<br />To create new one - create directory in \'www\'.',
...
if (is_dir($aliasDir)) // =../alias/
{
    ...
    { // npr $file = dev.conf -> dev
	    ...
	    {		
          ...
          if ($podmapa == 'dev') {
              $urlbit = 'http://'.$podmapa ;
              $podmapa = $podmapa . ' (alias i vhost)';
          };
              // apache sada nece dodati localhost/ ispred dev
		    $aliasContents .= 
          '<li>'.'<a href="'.$urlbit.'">'.$podmapa.'</a>'.'</li>';
	    }
    }
    closedir($handle);
}
...
#head {
	...
	text-indent: -500em; */
	height: 125px;
	/* kad se ovo otkomentira pokaze logo
...
dd {
	/* margin: -2% 0% 0% 20%; */ /* 12=lijeva margina oko 0,15 sirine 
            vrijedi za sva polja ispod naslova - verzije SW */
...
<body>
   ...
   <!-- SW verzije -->
	...
	<dl class="xxcontent">
      {$langues[$langue]['versa']}
		${apacheVersion}&nbsp;&nbsp;-&nbsp;<a href='http://{$langues[$langue][$doca_version]}'>Documentation</a>
      
		<br />{$langues[$langue]['versp']}
		${phpVersion}&nbsp;&nbsp;-&nbsp;<a href='http://{$langues[$langue]['docp']}'>Documentation</a>
      
		<br />{$langues[$langue]['server']}
		${server_software}
		
      <br />{$langues[$langue]['phpExt']}
		<dd><ul>${phpExtContents}</ul></dd>
		
      {$langues[$langue]['versm']}
		${mysqlVersion} &nbsp;-&nbsp; <a href='http://{$langues[$langue]['docm']}'>Documentation</a>
	</dl>
   ...
   \$wampConfFile = {$wampConfFile}  \$wampConfFileContents =<br />{$wampConfFileContents}
