...
if(isset($_POST['reset_request'])){
    ...
    if(!count($errors)){
        ...
        $objDB = objDB();
        $stmt = $objDB->prepare(
            "UPDATE admins SET is_active = '0', reset_code=? WHERE user_email=?"
        );
        $stmt->bind_param('ss', $code, $email);
        if($stmt->execute()){
            setMsg('msg', 'You made a password request, please check email to reset your password.', 'warning');

            $message = "Hi! You requested password reset, . You need to click here to <a href='".MODULE_URL."p_reset_code_verify.php?reset_code=$code'>Reset your password.</a>";
