<?php
// J:\awww\www\b12phpfw\Db_ allsites.php
...
{
  /**
  * DB T rait seems better (?) than abstract cls-es inheritance in B12phpfw because Home_ctr may inherit Config_ allsites which NOT extends Db_ allsites, so Home_ctr may work with more DB t rait. Solution without DB t raits in B12phpfw also works (is NOT simpler, better ?).
  *
...
// *************** FUNCTION 3. C R U D  S H A R E S  ***************
  ...
  */
  ...
  //eg $cursor_rowcnt = self::rr("SELECT COUNT(*) COUNT_ROWS FROM $tbl") ;
  //   $cursor_maxid = self::rr("SELECT max(id) MAXID FROM ". $tbl //." WHERE $qrywhere"
  //     , $binds=[], $other=['caller' => __FILE__ .' '.', ln '. __LINE__ ] ) ;
    
  static public function rr( $dmlrr, $binds = [], $other = [] ): object
  {
    /*
