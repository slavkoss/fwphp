  ...
  */
  ...
  public function __construct(object $pp1, array $pp1_module)
  {
    ...
    //1.r equirements_ok, 2.adresses (no constants), 3.routing, 4.dispatching
      ...
      /*see Autoload.php : if (strnatcmp(phpversion(),'5.4.0') >= 0) {
            if (s ession_status() == PHP_S ESSION_NONE) { s ession_start(); }
      } else { if(s ession_id() == '') { s ession_start(); } } */
           ...
           if (! function_exists('session_start'))
             { echo "<h2>PHP has no  s e s s i o n  support</h2>: 
             Your PHP installation doesn't have the 
             <a href=\"http://www.php.net/manual/en/ref.session.php\">Sess. module</a> 
  ...
  ,\'login\'           => QS.\'i/login/\'            METHOD login
      ...
      //$this->set_default_cls_states_atr($this->p p1);
                  ...
                  //echo '<b>$_S ESSION</b>='; print_r($_S ESSION); 
                  //
  ...
  //echo $user1->__get('age');
    ...
    // S E S S  I O N  M E T H O D S  were in cls in S essions.php
    ...
    //static public function M sgErr(){
      if(isset($_S ESSION["M sgErr"])){
        $Output = "<div class=\"alert alert-danger\">" ;
        $Output .= self::escp($_S ESSION["M sgErr"]);
        $Output .= "</div>";
        $_S ESSION["M sgErr"] = null;
        return $Output;
      }
    }
    //static public function M sgSuccess(){
      if(isset($_S ESSION["M sgSuccess"])){
        $Output = "<div class=\"alert alert-success\">" ;
        $Output .= self::escp($_S ESSION["M sgSuccess"]);
        $Output .= "</div>";
        $_S ESSION["M sgSuccess"] = null;
        return $Output;
      }
    } */
